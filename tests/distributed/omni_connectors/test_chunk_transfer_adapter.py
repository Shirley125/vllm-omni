...
def build_adapter(monkeypatch):
    def _build(*, stage_id: int = 1, model_mode: str = "ar", max_num_seqs: int = 2):
        ...
        adapter = OmniChunkTransferAdapter(SimpleNamespace(model_config=model_config, scheduler_config=scheduler_config))
...
def test_load_then_poll_ar_marks_chunk_finished(build_adapter):
...
def test_update_request_payload_merges_tensor_and_list(build_adapter):
...
def test_postprocess_scheduler_output_updates_cached_info_and_ready_set(build_adapter):
    adapter, _ = build_adapter()
