...
class InputStreamError(Exception):
    """Wrap errors raised by upstream input_stream generators."""
...
class AsyncOmni(EngineClient, OmniBase):
    ...
    async def generate(
        self,
        prompt: OmniPromptType | AsyncGenerator[StreamingInput, None],
    ...
    async def _add_streaming_input_request(
        ...
        *,
        ...
        # only check thinker's sampling params
        ...
        async def handle_inputs() -> None:
            ...
            try:
                ...
                await req_state.queue.put({"request_id": request_id, "error": InputStreamError(error)})
            finally:
                if not cancelled:
