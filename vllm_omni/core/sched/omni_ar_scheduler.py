...
class OmniARScheduler(VLLMScheduler):
    ...
    def _update_request_as_session(self, session: Request, update: StreamingUpdate) -> None:
        ...
        if self.vllm_config.model_config.stage_id != 0:
            ...
            #
            # IMPORTANT: Keep the same list object as at Request init time.
            # `session.all_token_ids` is a ConstantList built once from
            # `_all_token_ids`; replacing `_all_token_ids` with `del` + assign
            # leaves that view pointing at stale tokens (breaks scheduler
            # `_make_cached_request_data` and async worker RESUME_RECOVER).
