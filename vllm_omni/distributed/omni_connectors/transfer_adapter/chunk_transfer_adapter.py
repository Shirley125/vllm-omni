...
class OmniChunkTransferAdapter(OmniTransferAdapterBase):
    ...
    def load_async(self, request: Request):
        ...
        Args:
            request: Request that should receive the next upstream chunk.
                ``request.request_id`` is used for local bookkeeping and
                ``request.external_req_id`` is cached for cross-stage key
                construction. ``request.additional_information`` is initialized
                to ``None`` if the field is missing.
        """
    ...
    def save_async(
        ...
        """Build and enqueue one outbound chunk for asynchronous sending.

        Payload extraction is executed in the caller thread via
        ``custom_process_next_stage_input_func`` to avoid racing with mutable
        request state. If extraction fails or returns empty payload, nothing is
        queued and the chunk index is not advanced.

        Args:
            pooling_output: Current pooling tensor chunk used to build the
                connector payload for the next stage.
            request: Request that owns this chunk. ``request.external_req_id``
                is used as the sender-side request key.

        Notes:
            This method is non-blocking. Actual connector I/O is performed by
            the background ``save_loop`` thread.
        """
        if request is None:
            raise ValueError("request must be provided for chunk transfer")
